<template>
  <app>
    <v-container>
      ...
      <v-row justify="space-around">
        <v-col v-for="(item, index) of this.names" :key="index" :cols="columnSizes">
          <v-card :class="{ 'win-card': win, 'tie-card': tie }">
            <v-card-title><span style="color:orange">{{ win ? '¡Ganador!' : tie ? '¡Empate!' : '¡Vota Por!' }}</span>{{
              item.nombre
            }}</v-card-title>
            <v-card-subtitle>
              ...
              <v-row>
                <v-col>
                  ...
                  <v-tooltip bottom color="error">
                    <template v-slot:activator="{ on, attrs }">
                      <v-btn icon v-bind="attrs" v-on="on" @click="abrirDialogo(item._id)"
                        :disabled="maxVote"><v-icon>mdi-heart</v-icon></v-btn>
                    </template>
                ...
                <v-col>
                  <v-tooltip bottom color="success">
                    <template v-slot:activator="{ on, attrs }">
                      <v-btn icon v-bind="attrs" v-on="on" @click="comment(item)"
                        :disabled='calcularBuenos(item._id) == 0 && calcularMalos(item._id) == 0'><v-icon>mdi-comment</v-icon></v-btn>
                    </template>
                    <span>Visualizar los comentarios</span>
                  </v-tooltip>
                </v-col>

                <v-tooltip bottom>
                  <template v-slot:activator="{ on, attrs }">
                    <v-col><v-icon color="green" v-bind="attrs" v-on="on">mdi-account-plus</v-icon>{{
                      calcularBuenos(item._id) }}</v-col>
                  </template>
                ...
                <v-tooltip bottom>
                  <template v-slot:activator="{ on, attrs }">
                    <v-col><v-icon color="red" v-bind="attrs" v-on="on">mdi-account-remove</v-icon>{{
                      calcularMalos(item._id) }}</v-col>
                  </template>
      ...
      <v-overlay :value="overlay">
        <div class="text-center">
          ...
          <div style="font-size: 40px; margin: 5px">Reset Database</div>
        </div>
      </v-overlay>


      <v-dialog v-model="verComentarios" width="500px">
        <v-card class="text-left">
          <v-card-title class="text-center" style="font-size:20px;color:grey">Comentarios para {{ nombreComment }}
          </v-card-title>
          <v-divider></v-divider>
          <v-card-subtitle></v-card-subtitle>
          <v-card-text>

            <v-list-item two-line v-for="( item ) of  votesComment " :key="item._id">
              <v-list-item-avatar>
              ...
              <v-list-item-content>
                ...
                <v-list-item-subtitle>Valoración: <span v-if="item.value == 2" style="color:green;font-size:14px">{{
                  item.value }}</span>
                  <span v-else-if="item.value == -1" style="color:red;font-size:14px">{{ item.value
                  }}</span></v-list-item-subtitle>
              </v-list-item-content>
        ...
        <v-card>
          ...
          <v-card-actions class="d-flex justify-center">
            <v-btn dense class="success ma-2" @click="generarVoto(votante, 'voteOk', 'vote')" :loading="loadVoto"
              :disabled="!nombreValido || !comentarioValido"><v-icon>mdi-check</v-icon></v-btn>
            <v-btn dense color="error" @click="generarVoto(votante, 'voteNo', 'vote')" :loading="loadVoto"
              :disabled="!nombreValido || !comentarioValido"><v-icon>mdi-close</v-icon></v-btn>
          </v-card-actions>
        </v-card>
...
export default {
  ...
  methods: {
    ...
    async fetchData(collectionName) {
      ...
      try {
        this.loading = true
        const response = await fetch(`https://master--valley-dojo.netlify.app/api/data/${collectionName}`, { mode: 'no-cors' })
